...
function Users() {
  ...
  useEffect(() => {
    fetchHandler().then((data) => {
      if (data.Progressusers) {
        const filtered = data.Progressusers.filter((user) => user.Project_ID === "P001");
  ...
  const generatePDFReport = () => {
    ...
    const tableColumn = ["Project Name", "Description", "Duration", "Date", "Status", "Completion %"];
    const tableRows = [];
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "40px", maxWidth: "1400px", margin: "0 auto", backgroundColor: "rgba(255, 255, 255, 0.9)", borderRadius: "12px" }}>
        <h2 className="text-center mb-4" style={{ textAlign: "center", marginBottom: "30px" }}>Current Status of Your Project</h2>
          ...
          {p001Users.length > 0 ? (
            p001Users.map((user, index) => (
              ...
              >
                ...
                <h3 style={{ color: "#333", fontSize: "24px" }}>{user.Project_Name}</h3>
            ...
            <p style={{ textAlign: "center", color: "#333" }}>No progress records found.</p>
          )}
        </div>
      </div>
