...
function Monitoring() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            const res = await axios.get("http://localhost:5000/Monitoring", {
                signal,
            });
    ...
    useEffect(() => {
        if (searchTerm.trim() === "") {
            ...
            const filtered = records.filter((record) => {
                return (
                    record.Project_Name?.toLowerCase().includes(
                        searchTermLower
                    ) ||
                    record.Location?.toLowerCase().includes(searchTermLower) ||
                    record.Issues_Found?.toLowerCase().includes(
                        searchTermLower
                    ) ||
        ...
        <div className="page-container">
            ...
            <div className="users-container">
                <div className="users-header">
                    ...
                    <h1 className="users-title text-end">On-Site Monitoring</h1>
                </div>
